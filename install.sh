...
# installer/xiu_install.py, not here.

set -e

REPO_URL="https://github.com/yrpcaro/xiu.git"
DIR="${XDG_DATA_HOME:-$HOME/.local/share}/xiu"
# Where the pre-rename clone lives; moved over below when the new dir is free.
OLD_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/ricelin"
...
fetch() {
	mkdir -p "$(dirname "$DIR")"
	# A box installed before the rename carries its clone at the old path;
	# move it over when the new home is free so the checkout, its remotes
	# and any local state survive, then proceed as usual.
	if [ -d "$OLD_DIR/.git" ] && [ ! -d "$DIR/.git" ]; then
		step "Moving the pre-rename clone to $DIR"
		mv "$OLD_DIR" "$DIR" || say "  could not move the old clone, fetching fresh"
	fi
	if [ -d "$DIR/.git" ]; then
		step "Updating xiu in $DIR"
		git -C "$DIR" pull --ff-only || say "  could not fast-forward, using the current checkout"
	else
		step "Cloning xiu into $DIR"
...
main() {
	...
	[ "$(uname -s)" = Linux ] || die "xiu only installs on Linux"
	...
	if { [ -f "${XDG_CONFIG_HOME:-$HOME/.config}/hypr/.xiu-managed" ] \
	...
	exec python3 "$DIR/installer/xiu_install.py" --source "$DIR/configs" "$@"
}
